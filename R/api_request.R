...
make_request <- function(
  ...
  if (!is.null(stream_callback) || !is.null(get_download_path_callback)) {
    ...
    curl::handle_setheaders(
      h,
      .list = append(get_authorization_header(as_list = TRUE), headers)
    )
    ...
    tryCatch(
      ...
      error = function(e) {
        res_data <- curl::handle_data(h)
        if (
          (res_data$status_code == 503 || res_data$status_code == 0) &&
            retry_count < 10
        ) {
        ...
        if (res_data$status_code >= 400) {
          ...
          if (stop_on_error) {
            stop(str_interp(
              "Received HTTP status ${res_data$status_code} for path ${url}"
            ))
          } else {
            return(NULL)
          }
    ...
    if (is.null(get_download_path_callback)) {
      ...
      tryCatch(
        ...
        error = function(e) {
          if (
            "accept-ranges" %in% names(response_headers) && retry_count < 10
          ) {
            Sys.sleep(retry_count)
            args$retry_count <- args$retry_count + 1
    ...
    if (!is.null(download_path)) {
      handler <- httr::write_disk(download_path, overwrite = download_overwrite)
    } else if (!is.null(stream_callback)) {
      handler <- httr::write_stream(stream_callback)
    }
    ...
    if (
      !is.null(httr::headers(res)$'x-redivis-warning') &&
        is.null(globals$printed_warnings[[
          httr::headers(res)$'x-redivis-warning'
        ]])
    ) {
    ...
    if (method == "HEAD") {
      if (httr::status_code(res) >= 400) {
        response_content <- jsonlite::fromJSON(URLdecode(
          httr::headers(res)$'x-redivis-error-payload'
        ))
      }
    } else {
      ...
      if (
        !is.null(httr::headers(res)$'content-type') &&
          stringr::str_starts(
            httr::headers(res)$'content-type',
            'application/json'
          ) &&
          (httr::status_code(res) >= 400 || parse_response)
      ) {
        is_json <- TRUE
    ...
    if (httr::status_code(res) >= 400 && stop_on_error) {
      if (is_json) {
        stop(str_interp(
          "${response_content$error}_error: ${response_content$error_description}"
        ))
      } else {
