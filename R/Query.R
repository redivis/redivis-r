...
Query <- setRefClass(
  ...
  methods = list(
    ...
    to_sf_tibble = function(
      ...
      params <- get_query_request_params(
        .self,
        max_results,
        variables,
        geography_variable
...
get_query_request_params = function(
  instance,
  max_results,
  variables,
...
query_wait_for_finish <- function(previous_response, count = 0) {
  if (
    previous_response$status == 'running' ||
      previous_response$status == 'queued'
  ) {
    Sys.sleep(1)
    count = count + 1
    res <- make_request(
      method = 'GET',
      path = str_interp("/queries/${previous_response$id}")
    )
