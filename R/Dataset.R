  ...
  public = list(
    ...
    initialize = function(
      ...
      if (
        !is.null(version) &&
          version != "current" &&
          version != "next" &&
          !startsWith(tolower(version), "v")
      ) {
        version_arg <- str_interp("v${version}")
      } else if (
        is.null(version) &&
          !is.na(stringr::str_match(name, ":(v\\d+[._]\\d+|current|next)")[2])
      ) {
        version_arg <- stringr::str_match(
          name,
          ":(v\\d+[._]\\d+|current|next)"
        )[2]
      }
      ...
      reference_id <- ""
      reference_id_split <- stringr::str_split_fixed(
        stringr::str_replace_all(
          name,
          ":(v\\d+[._]\\d+|current|next|sample)",
          ""
        ),
        ":",
        Inf
      ...
      scoped_reference_val <- if (length(properties$scopedReference)) {
        ...
        str_interp("${parsed_name}${reference_id}${version_string}")
      }
    ...
    update = function(
      ...
      if (!is.null(public_access_level)) {
        payload <- append(
          payload,
          list("public_access_level" = public_access_level)
        )
      }
      if (!is.null(description)) {
