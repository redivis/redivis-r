  ...
  while (start_byte < file_size || start_byte == 0) {
    ...
    tryCatch(
      ...
      error = function(e) {
        if (retry_count > -1) {
          stop(str_interp(
            "A network error occurred. Upload failed after too many retries. Error: ${e}"
          ))
        }
...
parallel_download_raw_files <- function(
  instance,
  path = getwd(),
  overwrite = FALSE,
  max_results = NULL,
  ...
  if (progress) {
    progressr::with_progress(perform_parallel_raw_file_download(
      df[[file_id_variable]],
      path,
      overwrite,
      max_parallelization
    ...
    perform_parallel_raw_file_download(
      df[[file_id_variable]],
      path,
      overwrite,
      max_parallelization
...
perform_parallel_raw_file_download <- function(
  vec,
  path,
  overwrite,
  max_parallelization
  ...
  download_paths <- perform_parallel_download(
    purrr::map(vec, function(id) {
      str_interp("/rawFiles/${id}")
    }),
    overwrite = overwrite,
