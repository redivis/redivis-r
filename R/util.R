...
perform_resumable_upload <- function(data, temp_upload_url=NULL, on_progress=NULL) {
  retry_count <- 0
  start_byte <- 0
  file_size <- 0
  headers <- c()
...
perform_standard_upload <- function(data, temp_upload_url=NULL, retry_count=0, on_progress=NULL) {
