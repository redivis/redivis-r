  ...
  methods = list(
    ...
    download_files = function(path = NULL, overwrite = FALSE, progress = TRUE) {
      ...
      if (!overwrite && file.exists(path) && (!is_dir || file_count > 1)) {
        stop(sprintf(
          "File already exists at '%s'. Set parameter overwrite=TRUE to overwrite existing files.",
          path
        ))
      }
      ...
      if (progress && FALSE) {
        progressr::with_progress(perform_parallel_export_download(
          uri = .self$uri,
          download_path = path,
          file_count = file_count,
          is_dir = is_dir,
          overwrite = overwrite,
          total_size = .self$properties$size
        ))
      } else {
        perform_parallel_export_download(
          uri = .self$uri,
          download_path = path,
          file_count = file_count,
          is_dir = is_dir,
    ...
    wait_for_finish = function() {
      ...
      while (TRUE) {
        if (.self$properties$status == "completed") {
          ...
          stop(sprintf(
            "Export job failed with message: %s",
...
perform_parallel_export_download <- function(
  uri,
  file_count,
  download_path,
  is_dir,
  ...
  output_file_paths <- furrr::future_map(1:file_count, function(file_number) {
    ...
    if (is_dir) {
      headers_callback <- function(headers) {
        name <- get_filename_from_content_disposition(
          headers$'content-disposition'
        )
        final_path <<- file.path(download_path, name)
        if (!overwrite && base::file.exists(final_path)) {
          stop(str_interp(
            "File already exists at '${final_path}'. Set parameter overwrite=TRUE to overwrite existing files."
          ))
        }
