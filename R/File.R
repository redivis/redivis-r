...
File <- setRefClass(
  ...
  methods = list(
    initialize = function(
      ...
      populated_properties = append(
        properties,
        list(kind = "rawFile", id = id, uri = str_interp("/rawFiles/${id}"))
      )
      callSuper(
        ...,
        id = id,
    ...
    show = function() {
      if (!is.null(.self$properties$name)) {
        print(str_interp(
          "<File name=${.self$properties$name}, id=${.self$id})>"
        ))
      } else {
      ...
      if (is_dir) {
        get_download_path_callback <- function(headers) {
          ...
          if (!overwrite && base::file.exists(file_name)) {
            stop(str_interp(
              "File already exists at '${file_name}'. Set parameter overwrite=TRUE to overwrite existing files."
            ))
          }
          return(file_name)
        }
...
parse_file_headers <- function(file, headers) {
  file$properties$name <- get_filename_from_content_disposition(headers[[
    "content-disposition"
  ]])
  file$properties$contentType <- headers[["content-type"]]

  digest = if (!is.null(headers[["Digest"]])) {
    headers[["Digest"]]
  } else {
    headers[["x-goog-hash"]]
  }
  ...
  file$properties$size <- as.integer(dplyr::coalesce(
    headers[["x-redivis-size"]],
