  ...
  methods = list(
    ...
    create_output_table = function(
      ...
      res <- make_request(
        method = "POST",
        path = str_interp(
          "/notebookJobs/${current_notebook_job_id}/tempUploads"
        ),
        payload = list(
          tempUploads = list(list(
            size = file_size,
            resumable = file_size > 5e7
          ))
        )
      )
      ...
      res <- make_request(
        method = 'PUT',
        path = str_interp(
          "/notebookJobs/${current_notebook_job_id}/outputTable"
        ),
        payload = payload
      )
      ...
      table <- User$new(name = user_name)$workflow(name = workflow_name)$table(
        name = res$name
      )
